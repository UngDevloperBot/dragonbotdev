...
const ftnApi = new Fortnite("e876679e-9988-4374-8dc9-db484c66336f");
...
module.exports.run = async (bot, message, args) => {
  ...
  if (args[2]) {
    if (args[2].toLowerCase() == "all" || args[2].toLowerCase() == "season") {
      ...
      return message.channel.send(
        "Error. Use the right syntax: `d!fbr <epic-username> [platform pc/xbl/psn] {mode all/season}`.\nFor lifetime stats use `d!fbr <epic-username> [platform pc/xbl/psn]`"
      ); //Sends error message
    }
  }
  ...
  let data = ftnApi
    ...
    .catch(e => {
      ...
      //console.log(e);
      return message.channel.send(
        "Error. User not found, make sure you are using the right syntax: `d!fbr <epic-username> [platform pc/xbl/psn] {mode all/season}`.\nFor lifetime stats use `d!fbr <epic-username> [platform pc/xbl/psn]`\n\n**If this problem keeps arising, make sure you use the `.issue` command to report any issues with the bot**"
      ); //Send error message
    });
};
